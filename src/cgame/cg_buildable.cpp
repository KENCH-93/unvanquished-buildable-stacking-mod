...
static bool CG_GetBuildableRangeMarkerProperties(buildable_t bType, rangeMarker_t* rmType, float* range, Color::Color& rgba)
{
	...
	{
		// HACK: Assumes  the default trapper attack cone configuration
	...
	vec3_t outAxis[3], vec3_t outOrigin)
{
	...
	CG_CapTrace(&tr, inOrigin, mins, maxs, end, skipNumber, MASK_PLAYERSOLID, 0);
...
// Empilage coté client : place le ghost sur le buildable en-dessous
// comme le fait le serveur dans SpawnBuildable (EMPILAGE AU MOMENT DU SPAWN)
static void CG_StackGhostLikeServer(buildable_t buildable, vec3_t origin)
{
	if (!cg.snap)
		return;

	vec3_t gMins, gMaxs;
	BG_BuildableBoundingBox(buildable, gMins, gMaxs);

	// Bounding box du ghost dans le monde
	vec3_t ourMins, ourMaxs;
	VectorAdd(origin, gMins, ourMins);
	VectorAdd(origin, gMaxs, ourMaxs);

	float ourBottomZ = ourMins[2];

	// Trace vers le bas pour détecter si on est dans du solide
	...
	trace_t tr;
	CG_Trace(&tr, traceStart, gMins, gMaxs, traceEnd,
		cg.predictedPlayerState.clientNum, MASK_PLAYERSOLID, 0);

	if (!tr.startsolid)
		return; // Pas dans du solide, pas besoin de stacker

	// On est dans quelque chose - chercher le buildable le plus haut
	// qui OVERLAP réellement avec notre ghost
	float bestTopZ = -1e9f;
	bool found = false;
	...
	for (int i = 0; i < MAX_GENTITIES; i++)
	{
		...
		if (cent->currentState.eType != entityType_t::ET_BUILDABLE)
			continue;

		vec3_t obMins, obMaxs, oMin, oMax;
		BG_BuildableBoundingBox((buildable_t)cent->currentState.modelindex, obMins, obMaxs);
		VectorAdd(cent->lerpOrigin, obMins, oMin);
		VectorAdd(cent->lerpOrigin, obMaxs, oMax);

		// Vérifier overlap XY avec les VRAIES bounding boxes
		bool xOverlap = (ourMaxs[0] > oMin[0]) && (ourMins[0] < oMax[0]);
		bool yOverlap = (ourMaxs[1] > oMin[1]) && (ourMins[1] < oMax[1]);

		if (!xOverlap || !yOverlap)
		{
			// Pas d'overlap XY strict - essayer avec la distance horizontale
			float dx = origin[0] - cent->lerpOrigin[0];
			float dy = origin[1] - cent->lerpOrigin[1];
			float horizDist = sqrtf(dx * dx + dy * dy);

			// Taille horizontale combinée des deux buildables
			float ourRadius = (gMaxs[0] - gMins[0]) * 0.5f;
			float otherRadius = (obMaxs[0] - obMins[0]) * 0.5f;

			if (horizDist > ourRadius + otherRadius)
				continue; // Trop loin horizontalement
		}

		float otherTopZ = oMax[2];

		// Le sommet de l'autre buildable doit être AU-DESSUS de notre bas
		// (sinon il est en dessous et ne nous gêne pas)
		if (otherTopZ <= ourBottomZ)
			continue;

		Log::Notice("STACK: overlap ent#%d topZ=%.1f ourBottom=%.1f",
			i, otherTopZ, ourBottomZ);
	...
	if (found)
	{
		float newZ = bestTopZ - gMins[2];
		Log::Notice("STACK: moving from Z=%.1f to Z=%.1f (bestTop=%.1f gMinsZ=%.1f)",
			origin[2], newZ, bestTopZ, gMins[2]);
		origin[2] = newZ;
	}
}
...
/*
==================
CG_GhostBuildable - Ghost avec stacking infini (R Unlimited CZ style)
==================
*/

// maybe move this to shared/ and add a prototype?
bool IsMainBuildable(buildable_t buildable)
{
...
{
	const centity_t* main = CG_LookupMainBuildable();
	if (main)
	{
		return Distance(cg.predictedPlayerState.origin, main->lerpOrigin);
	}
	return 1E+37;
}
...
void CG_GhostBuildable(int buildableInfo)
{
	...
	if (buildable <= BA_NONE || buildable >= BA_NUM_BUILDABLES) return;

	const buildableModelConfig_t* bmc = BG_BuildableModelConfig(buildable);

	int reason = SB_BUILDABLE_TO_IBE(buildableInfo);

	vec3_t mins, maxs, angles, origin;
	trace_t tr;

	BG_BuildableBoundingBox(buildable, mins, maxs);
	BG_PositionBuildableRelativeToPlayer(ps, mins, maxs, CG_Trace, origin, angles, &tr);

	refEntity_t ent = {};
	ent.reType = refEntityType_t::RT_MODEL;
	ent.renderfx = RF_NOSHADOW;

	CG_PositionAndOrientateBuildable(angles, origin, tr.plane.normal, ps->clientNum,
		mins, maxs, ent.axis, ent.origin);

	// Stacking AVANT le zOffset
	CG_StackGhostLikeServer(buildable, ent.origin);

	// zOffset du modèle APRES le stacking
	...
	// Couleur basée uniquement sur la raison du serveur (pas de check clip local
	// car le stacking fait que le ghost est toujours "dans" le buildable en dessous)
	if (reason != IBE_NONE)
		ent.customShader = cgs.media.redBuildShader;
	else
	...
	// Skeleton pour modèles MD5/IQM
	if (cg_buildables[buildable].md5)
	{
		animation_t* anim = &cg_buildables[buildable].animations[BANIM_IDLE1];
		if (anim->handle)
		{
			trap_R_BuildSkeleton(&ent.skeleton, anim->handle, anim->firstFrame, anim->firstFrame, 0.0f, anim->clearOrigin);
