...
/**
 *briefSets the client side buildable idle animation
 */
...
/**
 * Returns whether two buildables placed at the given origins would overlap.
 *
 * This helper is used by custom buildable replacement and stacking logic.
 */
static bool BuildablesIntersect(buildable_t a, vec3_t originA,
	buildable_t b, vec3_t originB)
{
	// calcule la bbox des deux entites
	vec3_t minsA, maxsA, minsB, maxsB;

	BG_BuildableBoundingBox(a, minsA, maxsA);
	BG_BuildableBoundingBox(b, minsB, maxsB);

	// translate selon les positions
	VectorAdd(originA, minsA, minsA);
	VectorAdd(originA, maxsA, maxsA);
	VectorAdd(originB, minsB, minsB);
	VectorAdd(originB, maxsB, maxsB);

	//Dtectersans bloquer les collisions
	bool overlap = !(maxsA[0] <= minsB[0] || minsA[0] >= maxsB[0] ||
...
{
	// dont replace the main buildable with any other buildable
	...
	// dont replace last spawn with a non-spawn
	...
	vec3_t origin, vec3_t normal, int* groundEntNum)
{
	...
	// ============================
	// EMPILAGE approche par trace
	// ============================
	{
		trace_t stackTrace;
		vec3_t stackEnd;
		VectorCopy(entity_origin, stackEnd);
		stackEnd[2] -= 1.0f;
		trap_Trace(&stackTrace, entity_origin, mins, maxs, stackEnd, ENTITYNUM_NONE, MASK_PLAYERSOLID, 0);

		if (stackTrace.startsolid)
		{
			vec3_t ourMins, ourMaxs;
			VectorAdd(entity_origin, mins, ourMins);
			VectorAdd(entity_origin, maxs, ourMaxs);
			float ourBottomZ = ourMins[2];

			float bestTopZ = -1e9f;
			bool found = false;

			for (gentity_t* other = &g_entities[MAX_CLIENTS]; other < &g_entities[level.num_entities]; other++)
			{
				if (!other->inuse)
					continue;
				...
				vec3_t obMins, obMaxs, oMin, oMax;
				BG_BuildableBoundingBox((buildable_t)other->s.modelindex, obMins, obMaxs);
				VectorAdd(other->s.origin, obMins, oMin);
				VectorAdd(other->s.origin, obMaxs, oMax);

				bool xOverlap = (ourMaxs[0] > oMin[0]) && (ourMins[0] < oMax[0]);
				bool yOverlap = (ourMaxs[1] > oMin[1]) && (ourMins[1] < oMax[1]);

				if (!xOverlap || !yOverlap)
				{
					float dx = entity_origin[0] - other->s.origin[0];
					float dy = entity_origin[1] - other->s.origin[1];
					float horizDist = sqrtf(dx * dx + dy * dy);
					float ourRadius = (maxs[0] - mins[0]) * 0.5f;
					float otherRadius = (obMaxs[0] - obMins[0]) * 0.5f;
					if (horizDist > ourRadius + otherRadius)
						continue;
				}

				float otherTopZ = oMax[2];
				if (otherTopZ <= ourBottomZ)
					continue;

				if (otherTopZ > bestTopZ)
				{
					bestTopZ = otherTopZ;
					found = true;
				}
			...
			if (found)
			{
				float newZ = bestTopZ - mins[2];
				entity_origin[2] = newZ;
			}
		}
	}
	// ============================

	// Verifier que la position finale n est pas dans le monde
	trap_Trace(&tr2, entity_origin, mins, maxs, entity_origin, ENTITYNUM_NONE, MASK_PLAYERSOLID, 0);

	VectorCopy(entity_origin, origin);
	*groundEntNum = tr1.entityNum; // on laisse comme avant, pas critique pour les piles
	VectorCopy(tr1.plane.normal, normal);
	minNormal = BG_Buildable(buildable)->minNormal;
	invert = BG_Buildable(buildable)->invertNormal;

	// Custom stacking mode currently relaxes the vanilla surface normal check. on peut construire sur nimporte quelle pente
/*
...
//  Custom stacking mode currently allows placement on non-world support entities
/*
...
*/
	...
	else if (ent->client->pers.team == TEAM_ALIENS)
	{
		...
		// Check surface permissions temporarily disabled for the custom stacking workflow
			/*
	...
	else if (ent->client->pers.team == TEAM_HUMANS)
	{
		...
		// Check permissions de surface temporarily disabled for the custom stacking workflow
		/*
	...
	//modif serv : ignore collisions with buildables


	if (tr2.fraction < 1.0f)
	{
		// Si c�est un mur/sol du monde faut bloquer
		if (tr2.entityNum == ENTITYNUM_WORLD)
			reason = IBE_NOROOM;
	}

	// tr3 removed - was uninitialized
	...
	const vec3_t normal, const vec3_t angles, int groundEntNum)
{
	...
	// ============================
	//  EMPILAGE AU MOMENT DU SPAWN
	// ============================
	{
		// Notre bbox actuelle
		vec3_t ourMins, ourMaxs;
		VectorAdd(built->s.origin, built->r.mins, ourMins);
		VectorAdd(built->s.origin, built->r.maxs, ourMaxs);

		float ourMinX = ourMins[0];
		float ourMaxX = ourMaxs[0];
		float ourMinY = ourMins[1];
		float ourMaxY = ourMaxs[1];
		float ourBottomZ = ourMins[2];

		gentity_t* best = nullptr;
		float bestTopZ = -1e9f;

		// Rayon de recherche autour de la position de spawn
		float radius = std::max(built->r.maxs[0] - built->r.mins[0],
			built->r.maxs[1] - built->r.mins[1]) * 1.5f;

		gentity_t* other = nullptr;
		while ((other = G_IterateEntitiesWithinRadius(
			other, VEC2GLM(built->s.origin), radius)))
		{
			if (other == built || other->s.eType != entityType_t::ET_BUILDABLE)
			{
				continue;
			}
			...
			float otherMinX = oMin[0];
			float otherMaxX = oMax[0];
			float otherMinY = oMin[1];
			float otherMaxY = oMax[1];
			float otherTopZ = oMax[2];

			// pas de chevauchement en XY  pas un candidat pour empiler
			if (ourMaxX <= otherMinX || ourMinX >= otherMaxX ||
				ourMaxY <= otherMinY || ourMinY >= otherMaxY)
			{
				continue;
			}

			// On veut un buildable sous nous, avec son top au-dessus de notre bas actuel
			if (otherTopZ > ourBottomZ && otherTopZ > bestTopZ)
			{
				bestTopZ = otherTopZ;
				best = other;
			}
		...
		// Si no support remonte le nouveau pile sur son sommet 
		if (best)
		{
			float deltaZ = bestTopZ - ourBottomZ;

			built->s.origin[2] += deltaZ;
			built->s.pos.trBase[2] += deltaZ;
			built->r.currentOrigin[2] += deltaZ;

			// recalcule notre bbox pour la suite
			VectorAdd(built->s.origin, built->r.mins, ourMins);
			VectorAdd(built->s.origin, built->r.maxs, ourMaxs);
			ourBottomZ = ourMins[2];

			// force un petit drop standard now
			groundEntNum = ENTITYNUM_NONE;
		}
	}
	// ============================
	//  FIN EMPILAGE SPAWN
	...
	//  These are preliminary angles. The real angles of the model are calculated client side.
	...
	if (built->r.maxs[2] - built->r.mins[2] > 47.0f) // HACK: Fixed jump height
	{
		...
		// fixVal was choosen to work on all known cases:
